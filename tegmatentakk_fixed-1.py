...
def extract_amount_from_text(text):
    ...
    patterns = [
        ...
        # Umumiy raqam patterni oxirgi navbatda va faqat aniq narx formatiga
        # o'xshasa ishlatiladi (masalan 1,000 yoki 10.000 kabi ajratkichli sonlar),
        # sana (23.07.2026) yoki karta raqami (16 xonali) bilan aralashmasligi uchun.
        r'(\d{1,3}(?:[\s,.]\d{3})+)',
    ]
...
async def process_deposit_amount(message: Message, state: FSMContext):
    ...
    if amount > 100_000_000:
        await message.answer("❌ Summa juda katta. Qayta kiriting:")
        return
...
async def process_receipt(message: Message, state: FSMContext):
    ...
    if success:
        ...
        if deposit_id is None:
            await message.answer(
                "❌ Chekni saqlashda xatolik yuz berdi. Iltimos, qayta urinib ko'ring yoki admin bilan bog'laning: "
                f"{ADMIN_USERNAME}",
